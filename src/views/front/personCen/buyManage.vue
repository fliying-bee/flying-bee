                        ...
                        <div v-for="order in buyList">
                            <div v-if="order.buyStatus!='cancel'">
                                <Row type="flex" align="middle" class="front-order-item-title1">
                                    ...
                                    <i-col span="4">
                                        订单号：<span>{{order.buyId}}</span>
                                    </i-col>
                                    <i-col span="2">
                                        <span v-if="order.buyStatus=='notpay'">未付款</span>
                                        <span v-if="order.buyStatus=='paid'">已付款</span>
                                    </i-col>
                                    <i-col span="2" v-if="order.buyStatus=='paid'">
                                    ...
                                    <i-col span="3">
                                        金额：<span>￥{{order.buyPriceSum}}</span>
                                    </i-col>
                                </Row>
                                ...
                                <Row type="flex" align="middle" justify="center" class="front-order-item-content"
                                     v-for="product in order.buyDetail">
                                    <i-col span="2">
                                ...
                                <div class="front-order-title clearfix" style="border-top:none;">
                                    <div class="front-order-btn">
                                        <i-button type="primary"
                                                  v-if="order.buyStatus=='paid'&&order.logisStatus!='received'"
                                                  @click="updateBuyLogisStatus(order.buyId)">收货</i-button>
                                        <i-button type="primary" v-if="order.buyStatus=='notpay'"
                                                  @click="updateBuyStatus(order.buyId,'paid')">付款</i-button>
                                        <i-button type="ghost" v-if="order.buyStatus=='notpay'"
                                                  @click="updateBuyStatus(order.buyId,'cancel')">取消订单</i-button>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    export default {
        ...
        methods: {
            ...
            queryAllBuyPage(){
                ...
                self.$http({
                    method: 'GET',
                    url: 'http://127.0.0.1:8080/Spring-study/queryAllBuyPage',
                    params:data
                }).then(function (res) {
                    if (res.data.code == "OK") {
            ...
            updateBuyLogisStatus(buyId){
                var self = this;
                var data = {
                    buyId:buyId,
                    logisStatus:'received'
                };
                self.$http({
                    method:'POST',
            ...
            updateBuyStatus(buyId,buyStatus){
                var self = this;
                var data = {
                    buyId:buyId,
                    buyStatus:buyStatus
                ...
                self.$http({
                    method:'POST',
                    url:'http://127.0.0.1:8080/Spring-study/updateBuyStatus',
                    params:data
                }).then(function(res){
                    if(res.data.code=="OK"){
                        self.queryAllBuyPage();
